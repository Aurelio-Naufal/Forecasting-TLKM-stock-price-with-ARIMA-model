...
library(lmtest)
#### Import Data ####
tlkm <- read.csv("E:\\Documents\\OneDrive - UNIVERSITAS INDONESIA
\\Semester 4\\[SCST602201] Metode Peramalan\\Dataset\\TLKM.JK.csv")
...
legend("topleft",
       ...
       lwd = 2,
       bty = "n")
fit <- Arima(ts, order = c(0,1,0), include.constant = FALSE, lambda = 
               BoxCox.lambda(ts))
...
autoplot(win, main = "Peramalan ARIMA(0,1,0) 4 Bulan ke Depan",
         xlab = "Waktu",
         ylab = "Harga penutupan",
         lwd = 1) +
